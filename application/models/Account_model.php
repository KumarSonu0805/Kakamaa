...
class Account_model extends CI_Model{
	...
	public function createinitialotp($data){
		...
		$query = $this->db->get_where('users',$where);
		if($query->num_rows()==0){
            $expiry=date('Y-m-d H:i:s',strtotime('-16 minutes'));
            $this->db->update('user_otp',['status'=>2],"role='$data[role]' and status='0' and updated_on<='$expiry'");
	...
	public function addtoken($data,$type="multiple"){
        if(isset($_SERVER['HTTP_HOST']) && ($_SERVER['HTTP_HOST']=='localhost' || $_SERVER['HTTP_HOST']=='192.168.1.104' || $_SERVER['HTTP_HOST']=='192.168.29.104')){
            $type='multiple';
        }
        if($type!="multiple"){
	...
	public function verify_token($token){
		...
		if($gettoken->num_rows()==1){
			...
			if(is_array($array)){
                ...
                if($array['role']=='member'){
                    $this->wallet->addcommission($user_id);
                }
                elseif($array['role']=='franchise'){
                    $this->wallet->addfranchisecommission($user_id);
                }
                $date=date('Y-m-d');
                $this->wallet->royaltyincome($date);
                $this->wallet->fundincome($date);
                $this->wallet->referralincome($date);
				return $array;
			}
			else{
				return false;
    ...
    public function getuser($where){
        $default=file_url('assets/images/avatar.png');
